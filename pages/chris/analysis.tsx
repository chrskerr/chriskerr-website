...
import ComparisonGraph from 'components/chris/comparison-graph';

interface IChrisNeckAnalyse {
	data: IData[];
}
...
function createRatios(input: {
	...
	return {
		has: input.hadPain / input.painDays || 0,
		not: input.noPain / (input.totalDays - input.painDays) || 0,
	};
}
...
export default function ChrisAnalyse({ data }: IChrisNeckAnalyse) {
	const today = formatISO(new Date(), { representation: 'date' });

	const [neckThreshold, setNeckThreshold] = useState<1 | 2 | 3>(2);
	const [threshold, setThreshold] = useState<1 | 2 | 3>(3);

	const summaryByLabel = createSummaryByLabel(data);
	const summaryByTag = createSummaryByTag(data, neckThreshold, threshold);
		...
		<>
			<div className="display-width">
				...
				<div className="flex items-center justify-between w-full">
					<div className="flex items-center">
						<Link href={`/chris/${today}`} passHref>
			...
			<div className="display-width divider-before">
						...
						{tags
							...
							.map(tag => {
								...
								const { has, not } = createRatios({
									...tagData,
									painDays: summaryByLabel[1][neckThreshold],
				...
				<p className="pb-2 text-sm">
					* For each stiumuli, the percentage of days which Chris had
					an sort neck (or not sore neck) after having experienced it
					on that same day or the day before.
				</p>
				<p className="pb-2 text-sm">
