...
async function getBirdEl(
	...
	return (user?: Element): Element => {
		...
		const x = isLeftFacing ? canvas.width : 0;
		const xVelocity = getValueFromRange(isLeftFacing ? -0.4 : 0.4, 0.1);
...
export async function initBirds(canvas: HTMLCanvasElement) {
	const birdEls = [
		await getBirdEl(canvas, 'hummingbird.svg', true),
		await getBirdEl(canvas, 'hummingbird.svg', false),
	];
