...
export function refitCanvasToScreen() {
	...
	const maxHeight =
		window.innerHeight - canvas.offsetTop - footer.offsetHeight;

	if (parentEl) {
		canvas.width = Math.min(
			Math.min(parentEl.clientWidth, window.innerWidth, maxGameWidth),
		);
...
export const getHeightFromTargetWidth = (
	el: HTMLImageElement,
	targetWidth: number,
) => {
