...
const minifyDayObject = (day: Day): MinifiedDay => {
	return {
		...
		s: day.repSplit,
		t:
			day.swingType === Type.ONE_HANDED
				? MinifiedType.ONE_HANDED
				: MinifiedType.TWO_HANDED,
	};
};
...
export const encode = (data: Day | Week): string | undefined => {
	...
	if (Array.isArray(data)) {
		const minifiedData = data.map(day =>
			day ? minifyDayObject(day) : null,
		);
...
export const decode = (input: string | string[] | undefined) => {
	...
	try {
		...
		const data = jsonString
			? (JSON.parse(jsonString) as MinifiedWeek | MinifiedDay)
			: null;
